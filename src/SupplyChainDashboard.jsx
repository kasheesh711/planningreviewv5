...
const NetworkGraphView = ({ rawData, bomData, isDarkMode, selectedNode }) => {
    ...
    useEffect(() => {
        ...
        const tick = () => {
            ...
            ctx.lineWidth = 1;
            links.forEach(link => {
                const isConnectedToSelection = selectedIdStr && (link.source.id === selectedIdStr || link.target.id === selectedIdStr);
                ctx.beginPath();
                ctx.moveTo(link.source.x, link.source.y);
                ctx.lineTo(link.target.x, link.target.y);
                if (isConnectedToSelection) {
                    ctx.strokeStyle = isDarkMode ? 'rgba(99,102,241,0.6)' : 'rgba(79,70,229,0.6)';
                    ctx.lineWidth = 2;
                } else {
    ...
    }, [graphData, hideOrphans, metricMode, isDarkMode, hoveredNode, selectedNode]);

    // Interaction
    const handleMouseMove = (e) => {
        // Placeholder for advanced interactivity if needed later
    };
